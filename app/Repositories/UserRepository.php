...
class UserRepository implements UserRepositoryInterface
{
    public function dataTableUser()
    {
        ...
        ->join('usershub', 'users.users_id', '=', 'usershub.users_id')
        ->join('hub', 'usershub.hub_id', '=', 'hub.hub_id')
        ->join(DB::raw("(
            SELECT users_id, ROW_NUMBER() OVER (ORDER BY users_id) AS row_index FROM users
        ...
        ->select('sub.row_index', 'users.full_name', 'users.username', 'hub.name as hub', 'role.name as role', 'users.email as email', 'users.is_active', 'sub2.status')
