...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card card-custom" id="card-reporting">
        ...
        <div class="card-body p-0">
            <div class="row">
                <div class="col-md-12">
                    <h5 class="title-adjustment">Reject Master Waybill</h5>
                </div>
                <div class="col-md-12">
                    <div class="d-flex align-items-center d-flex justify-content-between search-data" id="search-data">
                        <strong class="flex-fill">Master&nbsp;Waybill&nbsp;ID</strong>
                        <input type="text" name="waybill-code" placeholder="Master Waybill ID" id="waybill-code" class="form-control flex-fill">
                        <button type="button" class="btn btn-primary flex-fill" id="submitMaster">Submit</button>
                    </div>
                </div>
                <form action="{{ route('adjustment.master-waybill') }}" method="post" class="form-adjustment d-none needs-validation" id="form-adjustment" novalidate>
                    @csrf
                    <input type="hidden" name="id" id="input-id">
                    <div class="col-md-12">
                        <h5 class="detail-adjustment">Master Waybill Information</h5>
                    </div>
                    <div class="detail-waybill">
                        <div class="col-md-12">
                            <strong>Master Waybill ID:</strong>
                            <span id="master-waybill-id"></span>
                        </div>
                        <div class="col-md-12">
                        ...
                        <div class="col-md-12">
                            <strong>Total Waybill:</strong>
                            <span id="total-waybill"></span>
                        </div>
                    </div>
                        ...
                        <table id="DataTableHistory" class="table table-history">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {{ $adjustment->masterWaybill->package()->count() }}
                                    </td>
...
<script>
    $('#search-date').change(function()
    {
        ...
        window.location.href = "{{ route('adjustment.master-waybill') }}?date="+date+"&origin_filter="+hub
    });
    ...
    $(document).ready(function() {
        ...
        $("#submitMaster").click(function(){
            ...
            if (waybillCode.length == 0) {
                Swal.fire({
                    title: 'Error!',
                    text: 'Please input waybill master!',
                    icon: 'error',
                    customClass: {
                ...
                var url = "{{ route('adjustment.master-waybill-information') }}";
                $.ajax({
                    url: url,
                    method: 'POST',
                    data: {
                    ...
                    success: function(data) {
                        ...
                        if (data.success) {
                            ...
                            $("#master-waybill-id").html(master.code);
                            $("#created-date").html(master.created_date);
                            $("#created-by").html(master.created_by);
                            $("#total-waybill").html(master.total_waybill);
                            $("#input-id").val(master.master_waybill_id);
            ...
            function (event) {
                if (!form.checkValidity()) {
                    ...
                    Swal.fire({
                        ...
                        text: "Are you sure wants to reject Master Waybill?",
